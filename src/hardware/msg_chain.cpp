...
bool msg_chain_delete_msg_entry(msg_chain_t *msg_chain, int32_t entry, int32_t id)
{
    ...
    do
    {
        ...
        snprintf(buffer, 10, "\"id\":%d,", id);
                    Serial.println(buffer);

        if ((entry == msg_counter && id == -1) || (id > -1 && strstr(msg_chain_entry->msg, buffer) != NULL))
        {
            Serial.print("mazu:");
            Serial.println(id);
...
const char *msg_chain_get_msg_entry(msg_chain_t *msg_chain, int32_t entry, int32_t id)
{
    ...
    do
    {
        ...
        {
            snprintf(buffer, 10, "\"id\":%d,", id);
            if (strstr(msg_chain_entry->msg, buffer) != NULL)
            {
                retval = msg_chain_entry->msg;
                break;
            }
